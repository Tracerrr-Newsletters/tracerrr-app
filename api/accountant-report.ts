...
// VAT inference for costs (incoming transactions)
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    // Fetch matched incoming invoices for these transactions
    ...
    const incomingInvoiceMap = new Map((incomingInvoices ?? []).map(i => [i.id, i]));
    const missingInvoiceCount = transactions.filter(t => !t.invoice_id).length;

    // Warn if missing invoices and no override
    ...
    // ── 2. Fetch revenue (outgoing invoices) ──────────────────────────────────
    ...
    // Fetch sponsor names
    const sponsorIds = [...new Set(outgoing.map(o => o.sponsor_id).filter(Boolean))];
    ...
    if (hasDriveCredentials) {
      try {
        ...
        // Upload incoming invoice PDFs (costs)
        for (const inv of incomingInvoices ?? []) {
          ...
          const filename = `COST-${inv.invoice_number ?? inv.id}.pdf`;
          await uploadToDrive(token, quarterId, filename, bytes);
        }

        // Upload outgoing invoice PDFs (revenue)
        for (const inv of outgoing) {
          if (!inv.pdf_storage_path) continue;
          const { data: file } = await supabase.storage.from('invoices').download(inv.pdf_storage_path);
          if (!file) continue;
          const bytes = new Uint8Array(await file.arrayBuffer());
          const filename = `REVENUE-${inv.invoice_number ?? inv.id}.pdf`;
          await uploadToDrive(token, quarterId, filename, bytes);
        }
    ...
    y -= 18;
    draw(`Period: ${fmtDate(dateFrom)} — ${fmtDate(dateTo)}`, MARGIN, y, 9, false, rgb(0.4, 0.4, 0.4));
    y -= 13;
    ...
    // Column headers
    let x = MARGIN;
    COST_HEADERS.forEach((h, i) => {
      draw(h, x, y, 8, true, rgb(0.35, 0.35, 0.35));
      x += COST_COLS[i];
    });
    y -= 8;
    hRule(0.5, 0.8);
    ...
    let totalCostUSD = 0;
    let totalVatPaidUSD = 0;
    let totalVatPaidGBP = 0;
    let totalCostGBP = 0;
    ...
    for (const txn of transactions) {
      ...
      if (currency === 'GBP') {
        totalCostGBP += amount;
        if (vatAmount != null) totalVatPaidGBP += vatAmount;
      } else {
        totalCostUSD += amount;
        if (vatAmount != null) totalVatPaidUSD += vatAmount;
      }

      const hasInvoice = !!inv;
      const invoiceLabel = inv ? (inv.invoice_number ?? '✓') : 'Missing';
      ...
      costRow.forEach((col, i) => {
        draw(col.text, x, y, 9, false, col.color ?? rgb(0, 0, 0), col.maxW);
        x += COST_COLS[i];
      });
    ...
    if (totalVatPaidUSD === 0 && totalVatPaidGBP === 0) {
      draw('Review ? rows above', costTotX, y, 9, false, rgb(0.6, 0.4, 0.1));
    }

    if (missingInvoiceCount > 0) {
      y -= 14;
      draw(`⚠ ${missingInvoiceCount} transactions have no matched invoice (shown in red).`, MARGIN, y, 9, false, rgb(0.8, 0.2, 0.1));
    }
    ...
    for (const inv of outgoing) {
      ...
      const statusColor = inv.status === 'paid' ? rgb(0.1, 0.5, 0.2)
        : inv.status === 'overdue' ? rgb(0.8, 0.2, 0.1)
        : rgb(0, 0, 0);
    ...
    draw(
      totalVatChargedUSD > 0 ? fmtMoney(totalVatChargedUSD, 'USD') : 'Review ? rows — VAT unknown for some clients',
      revTotX + 80, y, 9, true, rgb(0.07, 0.39, 0.46)
    );
    ...
    page.drawRectangle({
      ...
      borderColor: rgb(0.07, 0.39, 0.46),
      borderWidth: 1,
    });
    ...
    draw(`− ${fmtMoney(vatPaidUSD, 'USD')}`, MARGIN + 320, y, 9, true);
    y -= 12;
    ...
    if (totalVatPaidGBP > 0 || totalVatChargedUSD === 0) {
      y -= 14;
      draw('⚠ Some VAT amounts marked ? — review with accountant before filing.', MARGIN + 12, y, 8, false, rgb(0.6, 0.4, 0.1));
    }
    ...
    const emailRes = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        ...
        subject: `VAT Report: ${fmtDate(dateFrom)} — ${fmtDate(dateTo)}`,
        html: `
          <p>Hi Jake,</p>
          <p>Please find attached your VAT report for <strong>${fmtDate(dateFrom)} — ${fmtDate(dateTo)}</strong>.</p>
          ...
          ${driveFolderUrl ? `<p>📁 <strong>All invoice PDFs:</strong> <a href="${driveFolderUrl}">View in Google Drive</a></p>` : ''}
